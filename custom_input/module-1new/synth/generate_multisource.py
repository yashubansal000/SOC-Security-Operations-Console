...
#   - JSON str:  what gets stored per-row in the `adjacent_nodes` /
#                `upstream` columns
# ===========================================================================
...
def topology_to_networkx(topology: dict[str, dict[str, Any]]) -> nx.Graph:
    ...
    care about traffic direction. Module 3/4 rebuild directed graphs from
    the same source dict (or from the persisted `upstream` column) when
    they need directionality.
...
def topology_to_dataframe(
    ...
    Persists BOTH:
      - `adjacent_nodes`: symmetric (undirected) neighbor list, derived
        from the graph — useful for general "is this node nearby" queries.
      - `upstream`: the true directed dependency list, taken straight from
        the topology dict itself. This is what Module 4's
        get_impact_path()/path_exists() need in order to distinguish
        upstream from downstream instead of treating every relationship
        as symmetric. Without this column, direction is lost once the
        data round-trips through SQLite.

    Args:
        topology: Dict from `build_topology` (or an equivalent dict built
            by Module 1's custom-upload extraction path).
        graph: The corresponding NetworkX graph (used only to compute the
            symmetric `adjacent_nodes` list).

    Returns:
        DataFrame with columns: node_id, tier, adjacent_nodes, upstream.
    """
    ...
    for node_id, meta in topology.items():
        ...
        records.append({
            ...
            "upstream": json.dumps(meta.get("upstream", [])),
        })
...
# NOTE: 'upstream' added so directed dependency info (which node a given
# node depends on) survives the round-trip to SQLite. Previously only the
# symmetric 'adjacent_nodes' list was persisted, which meant Module 4's
# upstream/downstream impact-path logic silently lost all directionality
# once the topology was read back out of the database.
_TOPOLOGY_TABLE_COLUMNS: tuple[str, ...] = (
    "node_id", "tier", "adjacent_nodes", "upstream",
)
